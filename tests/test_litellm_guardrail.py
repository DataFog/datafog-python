...
async def test_pre_call_redacts_tool_and_function_definitions():
            ...
            "json_schema": {
                ...
                "schema": {
                    ...
                    "properties": {
                        "email": {
                            "type": "string",
                            "description": f"For example, {EMAIL}",
                        },
                    },
                },
            },
    ...
    assert (
        CARD
        not in data["tools"][0]["function"]["parameters"]["properties"]["card"][
            "description"
        ]
    )
    assert SSN not in data["tools"][1]["description"]
    assert (
        EMAIL not in data["tools"][1]["input_schema"]["properties"]["email"]["examples"]
    )
    assert CARD not in data["functions"][0]["description"]
    assert SSN not in data["functions"][0]["parameters"]["properties"]["ssn"]["default"]
    assert (
        EMAIL
        not in data["response_format"]["json_schema"]["schema"]["properties"]["email"][
            "description"
        ]
    )
    ...
    assert (
        EMAIL
        in original["response_format"]["json_schema"]["schema"]["properties"]["email"][
            "description"
        ]
    )
...
async def test_block_raises_http_400_without_echoing_pii():
    guardrail = DataFogGuardrail(
        guardrail_name="datafog-pii", default_on=True, datafog_action="block"
    )
...
async def test_during_call_noop_when_action_is_redact():
    ...
    guardrail = DataFogGuardrail(
        guardrail_name="datafog-pii", default_on=True, datafog_action="redact"
    )
    data = _chat_data(f"reach me at {EMAIL}")
...
async def test_post_call_redacts_text_completion_choice_text():
    ...
    response = SimpleNamespace(
        choices=[SimpleNamespace(text=f"the customer email is {EMAIL}")]
    )
...
async def test_post_call_returns_original_when_event_hook_does_not_match():
    guardrail = DataFogGuardrail(
        guardrail_name="datafog-pii", default_on=True, event_hook="pre_call"
    )
    response = _model_response(f"the customer is reachable at {EMAIL}")
    ...
    assert (
        response.choices[0].message.content == f"the customer is reachable at {EMAIL}"
    )
...
async def test_post_call_skips_non_text_response_parts():
    guardrail = DataFogGuardrail(guardrail_name="datafog-pii", default_on=True)
    response = SimpleNamespace(
        choices=[SimpleNamespace(message=SimpleNamespace(content=[{"type": "image"}]))]
    )
    result = await guardrail.async_post_call_success_hook(
        data={}, user_api_key_dict=None, response=response
    )
...
async def test_streaming_redacts_model_response_chunks_before_yielding():
    ...
    chunks = [
        ModelResponseStream(
            choices=[{"index": 0, "delta": {"content": "email jane.doe@"}}]
        ),
        ModelResponseStream(
            choices=[
                {
...
async def test_entity_types_override():
    guardrail = DataFogGuardrail(
        ...
        default_on=True,
        datafog_entity_types=["IP_ADDRESS"],
    )
    data = await guardrail.async_pre_call_hook(
        user_api_key_dict=None,
        cache=None,
...
async def test_fail_closed_raises_without_pii_or_cause_chain(monkeypatch):
    guardrail = DataFogGuardrail(
        guardrail_name="datafog-pii", default_on=True, datafog_fail_policy="closed"
    )
    monkeypatch.setattr(
        "datafog.integrations.litellm_guardrail._redact_text",
    ...
    with pytest.raises(RuntimeError, match="fail_policy") as exc:
        await guardrail.async_pre_call_hook(
            user_api_key_dict=None,
            cache=None,
            data=_chat_data(f"reach me at {EMAIL}"),
...
async def test_post_call_block_raises_on_response_tool_call_arguments():
    ...
    assert exc.value.status_code == 400
    assert EMAIL not in str(exc.value.detail)
    assert response.choices[0].message.tool_calls[0].function.arguments == json.dumps(
        {"recipient": EMAIL}
    )
...
def test_invalid_config_rejected():
    with pytest.raises(ValueError):
        DataFogGuardrail(
            guardrail_name="datafog-pii", default_on=True, datafog_action="explode"
        )
    with pytest.raises(ValueError):
        DataFogGuardrail(
            guardrail_name="datafog-pii", default_on=True, datafog_fail_policy="maybe"
        )


@pytest.mark.asyncio
async def test_legacy_config_names_and_allowlists_remain_supported():
    ...
    data = await guardrail.async_pre_call_hook(
        user_api_key_dict=None,
        cache=None,
        data=_chat_data(f"contact {own_email} or {other_email}"),
        call_type="completion",
    )

    assert own_email in data["messages"][0]["content"]
    assert other_email not in data["messages"][0]["content"]


@pytest.mark.asyncio
async def test_prefixed_allowlist_patterns_remain_supported():
    guardrail = DataFogGuardrail(
        guardrail_name="datafog-pii",
        datafog_entity_types=["PHONE"],
        datafog_allowlist_patterns=[r"^555-0100$"],
    )

    data = await guardrail.async_pre_call_hook(
        user_api_key_dict=None,
        cache=None,
        data=_chat_data("call 555-0100"),
        call_type="completion",
    )

    assert data["messages"][0]["content"] == "call 555-0100"


@pytest.mark.asyncio
async def test_direct_adapter_usage_runs_by_default():
...
@pytest.mark.asyncio
async def test_explicit_default_off_requires_request_selection():
    guardrail = DataFogGuardrail(
        guardrail_name="datafog-pii",
        default_on=False,
        event_hook="pre_call",
    )
    original = _chat_data(f"email {EMAIL}")

    skipped = await guardrail.async_pre_call_hook(
        user_api_key_dict=None,
        cache=None,
        data=original,
        call_type="completion",
    )
    selected = await guardrail.async_pre_call_hook(
        user_api_key_dict=None,
        cache=None,
        data={**original, "guardrails": ["datafog-pii"]},
        call_type="completion",
    )

    assert skipped is original
    assert EMAIL in skipped["messages"][0]["content"]
    assert EMAIL not in selected["messages"][0]["content"]
