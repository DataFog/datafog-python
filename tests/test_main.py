...
from datafog.processing.text_processing.spacy_pii_annotator import SpacyPIIAnnotator as TextPIIAnnotator
...
def assert_annotation_results(annotated_text):
    ...
    assert "Travis Kalanick" in annotated_text["PERSON"], "Person not correctly identified"
    assert "1234 Elm St" in annotated_text["FAC"], "Facility not correctly identified"
    assert "Springfield" in annotated_text["GPE"], "Geopolitical entity not correctly identified"
...
def assert_file_output(annotated_text):
    ...
    with tempfile.NamedTemporaryFile(mode='w', delete=False) as temp:
        ...
        assert file_content == annotated_text, "File output doesn't match returned annotation"
    os.unlink(temp.name)
...
async def test_run_ocr_pipeline(mock_image_service, mock_text_service):
    ...
    mock_text_service.batch_annotate_text_async.return_value = {"PERSON": ["Satya Nadella"]}
    
    result = await datafog.run_ocr_pipeline(["image_url"])
    
    mock_image_service.ocr_extract.assert_called_once_with(["image_url"])
    mock_text_service.batch_annotate_text_async.assert_called_once_with(["Extracted text"])
...
async def test_run_text_pipeline(mock_text_service):
    ...
    result = await datafog.run_text_pipeline(["Elon Musk tries one more time to save his $56 billion pay package"])
    
    mock_text_service.batch_annotate_text_async.assert_called_once_with(["Elon Musk tries one more time to save his $56 billion pay package"])
...
def test_run_text_pipeline_sync(mock_text_service):
    ...
    mock_text_service.batch_annotate_text_sync.assert_called_once_with(["Jeff Bezos steps down as Amazon CEO"])
