...
def read_cldr_names(language, category):
    ...
    filename = data_filename(
        "cldr-json/cldr-json/cldr-localenames-full/main/{}/{}.json".format(
            language, category
        )
    )
...
def get_name_languages():
    cldr_main_path = Path(
        data_filename("cldr-json/cldr-json/cldr-localenames-full/main")
    )
    languages = [
        subpath.name
...
def get_population_data():
    ...
    for territory in territories:
        ...
        for language in territory:
            ...
            written_ls = (
                langcodes.get(l_code)
                .maximize()
                ._filter_attributes(["language", "script"])
            )
...
def build_data():
    ...
    with open("population_data.py", "w", encoding="utf-8") as outfile:
        ...
        write_python_dict(
            outfile, "LANGUAGE_WRITING_POPULATION", language_writing_population
        )
